...
class OCMNetGUI:
    VENDOR_LABELS = [
        ...
        ('H_3G', 'Huawei 3G'),
        ('H_4G', 'Huawei 4G'),
        ('H_PL', 'Huawei Packet Loss (4G)'),
        ('N_2G', 'Nokia 2G'),
        ('N_3G', 'Nokia 3G'),
        ('N_4G', 'Nokia 4G'),
        ('N_PL', 'Nokia Packet Loss (3G)'),
        ('Z_2G', 'ZTE 2G'),
        ('Z_3G', 'ZTE 3G'),
        ('Z_4G', 'ZTE 4G'),
        ('Z_PL', 'ZTE Packet Loss'),
    ]
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.file_vars = {key: tk.StringVar() for key, _ in self.VENDOR_LABELS}
        self.ocm_dir_var = tk.StringVar(
            value=os.path.join(os.getcwd(), 'files', 'ocm net')
        )
    ...
    def _build_ui(self) -> None:
        ...
        # OCM output folder
        ttk.Label(outer, text='OCM Output Folder:').grid(
            row=row, column=0, sticky='w', pady=3
        )
        ttk.Entry(outer, textvariable=self.ocm_dir_var).grid(
            row=row, column=1, sticky='ew', padx=6
        )
        ttk.Button(outer, text='Browse…', command=self._browse_ocm_dir).grid(
            row=row, column=2
        )
        row += 1
        ...
        # Vendor files section header + auto-detect
        ttk.Label(
            outer, text='Vendor Raw Files', font=('Segoe UI', 10, 'bold')
        ).grid(row=row, column=0, columnspan=2, sticky='w')
        ttk.Button(
            outer, text='Auto-detect from RAW FILES sub-folder',
            command=self._auto_detect,
        ).grid(row=row, column=2, sticky='e')
        row += 1

        # One row per vendor/generation
        for key, label in self.VENDOR_LABELS:
            ttk.Label(outer, text=f'{label}:').grid(
                row=row, column=0, sticky='w', pady=2
            )
            ttk.Entry(outer, textvariable=self.file_vars[key]).grid(
        ...
        self.run_btn = ttk.Button(
            outer,
            text='▶   Update OCM Reports (Daily + Weekly + Monthly)',
            command=self._run,
        )
        self.run_btn.grid(row=row, column=0, columnspan=3, pady=4)
        row += 1
        ...
        row += 1
        self.log_text = scrolledtext.ScrolledText(
            outer, height=12, state='disabled',
            font=('Consolas', 9), wrap='word',
        )
        self.log_text.grid(row=row, column=0, columnspan=3, sticky='nsew')
        outer.rowconfigure(row, weight=1)
    ...
    def _browse_ocm_dir(self) -> None:
        d = filedialog.askdirectory(initialdir=self.ocm_dir_var.get() or os.getcwd())
        if d:
            self.ocm_dir_var.set(d)

    def _browse_file(self, key: str) -> None:
        raw_dir = os.path.join(self.ocm_dir_var.get(), 'RAW FILES')
        init = raw_dir if os.path.isdir(raw_dir) else os.getcwd()
        fp = filedialog.askopenfilename(
            title=f'Select {key} vendor file',
            filetypes=[('Excel files', '*.xlsx *.xls'), ('All files', '*.*')],
    ...
    def _auto_detect(self) -> None:
        """Scan the RAW FILES sub-folder and populate file entries by prefix."""
        raw_dir = os.path.join(self.ocm_dir_var.get(), 'RAW FILES')
        if not os.path.isdir(raw_dir):
            messagebox.showwarning(
                'Folder not found',
                f'RAW FILES folder not found:\n{raw_dir}\n\n'
                'Check the OCM Output Folder setting.',
            )
            return

        # Maps vendor_gen key → tuple of filename prefixes to try
        # IMPORTANT: more-specific prefixes must come before overlapping ones
        patterns: dict[str, tuple] = {
            'H_2G': ('H 2G', 'H2G'),
            'H_3G': ('H 3G', 'H3G'),
            'H_4G': ('H 4G', 'H4G'),
            'H_PL': ('H PACKET',),             # 'H PACKET LOSS...' – before 'H P'
            'N_2G': ('N 2G', 'N2G'),
            'N_3G': ('N 3G', 'N3G'),
            'N_4G': ('N 4G', 'N4G'),
            'N_PL': ('N PACKET',),             # 'N PACKET LOSS...' – before 'N P'
            'Z_PL': ('Z PACKET',),             # 'Z PACKET LOSS...' – before 'Z P'
        ...
        for fname in sorted(os.listdir(raw_dir)):
            ...
            fpath = os.path.join(raw_dir, fname)
            for vg, pfxs in patterns.items():
                if any(fname.startswith(p) for p in pfxs):
                    if not self.file_vars[vg].get():
                        self.file_vars[vg].set(fpath)
                        found += 1
                    break
    ...
    def _run(self) -> None:
        ocm_dir = self.ocm_dir_var.get().strip()
        if not ocm_dir or not os.path.isdir(ocm_dir):
            messagebox.showerror('Error', f'OCM output folder not found:\n{ocm_dir}')
            return

        vendor_files = {key: self.file_vars[key].get().strip()
                        for key, _ in self.VENDOR_LABELS}
        provided = sum(1 for fp in vendor_files.values() if fp)
        if provided == 0:
            messagebox.showerror('Error', 'Please select at least one vendor file.')
        ...
        def _worker():
            try:
                process_vendor_files(vendor_files, ocm_dir, log_callback=self._log)
                self.root.after(
                    0,
                    lambda: messagebox.showinfo(
                        'Done',
                        'OCM reports updated successfully!\n\n'
                        'Daily, Weekly and Monthly files have been saved.',
                    ),
                )
            except Exception as exc:
                import traceback
